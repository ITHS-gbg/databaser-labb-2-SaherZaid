...
namespace BookstoreApp
{
    class Program
    {
        ...
        static void AddBook(BookStoresContext dbContext)
        {
            ...
            // Prompt for author selection or addition
            ...
            if (int.TryParse(authorInput, out int authorId))
            {
                ...
                else
                {
                    ...
                    AddAuthor(dbContext);
                    return; // Return to main menu after adding the new author
                }
            }
            else
            {
            ...
            // Collect other book details (language, price, release date, ISBN-13, author number)
...
static void AddAuthor(BookStoresContext dbContext)
        {
            ...
            if (int.TryParse(authorIdInput, out int authorId))
            {
                ...
                if (existingAuthor != null)
                {
                    ...
                    AddAuthor(dbContext); // Recursive call to retry adding an author
                    return;
                }
            }
            else
        ...
        static void RemoveBookFromStore(BookStoresContext dbContext)
        {
            ...
            // Display stores and prompt for selection

            // Display books in the selected store and prompt for selection

            // Remove the selected book from the store's inventory balance
        ...
        static void ListInventoryBalance(BookStoresContext dbContext)
        {
            ...
            // Retrieve and display inventory balance for each store
